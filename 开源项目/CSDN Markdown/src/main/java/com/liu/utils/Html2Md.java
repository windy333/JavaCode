...
public class Html2Md {
    ...
    private static String handleNode(Node node) {
        ...
        if ("p".equals(nodeName)) {
            ...
            if (pElement == null) {
                pElement = blockquoteElement.getElementsByTag("ol").first();
            }
            String pStr = pElement.html();
    ...
    private static String handleInnerHtml(String innerHTML, Element child) {
        ...
        if ("strong".equals(s)) {
            ...
            //因为csdn加了防盗链，根据src下载图片到本地，替换src
            String imageDir = FileUtils.downloadImage(imageSrc);
            innerHTML = "\n" + innerHTML.replace(child.toString(), "![img](" + imageDir + ")");
        } else if ("em".equals(s)) {
            innerHTML = innerHTML.replace(child.toString(), " *" + child.text() + "* ");
        } else if ("a".equals(s)) {
            ...
            StringBuilder olStr = new StringBuilder();
            // 处理ol中的li 类似于 > - test1
            Elements liElements = child.getElementsByTag("li");
            for (int i = 0; i < liElements.size(); i++) {
                Element li = liElements.get(i);
                //将<li>替换为空字符串
                olStr.append("> - ").append(li.html().replace("<li>", "")).append("\n");
            }
            innerHTML = olStr.toString();
        } else {
            innerHTML = innerHTML.replace(child.toString(), child.text());
        }
        return innerHTML;
